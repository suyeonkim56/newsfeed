...
public class CommentLikeService {
    ...
    @Transactional
    public void createCommentLike(AuthUser authUser, Long commentId) {
        ...
        if(commentLikeRepository.findByUserAndComment(findUser, comment).isPresent())
        {
    ...
    @Transactional
    public void deleteCommentLike(AuthUser authUser,Long commentId, Long likeId) {
        User findUser = userRepository.findById(authUser.getId())
                .orElseThrow(() -> new IllegalStateException("존재하지 않는 유저입니다."));
        Comment comment = commentRepository.findById(commentId)
                .orElseThrow(() -> new IllegalStateException("존재하지 않는 댓글입니다."));
